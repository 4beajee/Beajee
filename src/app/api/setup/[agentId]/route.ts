...
function buildSetupDocument(opts: {
  ...
  if (isOpenClawPlatform(platform)) {
    ...
    if (platform === "hermes") {
      sections.push(`Use Hermes cron with its gateway running. Create a recurring job that calls \`check_in({ agent_id: "${agentId}" })\`, delivers every inbox event through the owner's configured Hermes channel, and calls \`ack_inbox\` only after delivery. Follow \`next_check_in_ms\` from each response. The cron must be silent when \`inbox\` is empty: never send the owner a message about a successful heartbeat, online status, \`status: "alive"\`, or background matching work. On every empty-inbox run, its final response must be exactly \`[SILENT]\` (nothing else); Hermes recognizes that marker and suppresses cron delivery. Do not summarize matches, negotiations, beacons, reputation, or context status unless a new owner-facing inbox event requires a decision or message.\n`);
