...
function TelegramSocialBadge({ provider, profile }: { provider: "linkedin" | "twitter"; profile: { url: string; label: string } }) {
  ...
  return <a href={profile.url} target="_blank" rel="noopener noreferrer" className={`inline-flex min-h-11 items-center gap-2 rounded-full px-4 text-sm font-semibold text-white shadow-[0_14px_38px_rgba(0,0,0,0.38)] transition-transform active:scale-95 ${linkedIn ? "bg-[#0A66C2]" : "bg-[#1D9BF0]"}`} aria-label={`Open ${linkedIn ? "LinkedIn" : "Twitter / X"} profile ${profile.label}`}><span className="grid h-6 w-6 place-items-center rounded-md bg-white/15 text-[11px] font-bold">{linkedIn ? "in" : "𝕏"}</span>{profile.label}<span aria-hidden="true">↗</span></a>;
}
