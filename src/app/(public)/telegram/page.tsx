...
const TELEGRAM_LOCALES = [
  ["auto", "Auto-detect"],
  ["en", "English"],
  ["zh", "中文"],
  ["hi", "हिन्दी"],
] as const;
type TelegramLocale = (typeof TELEGRAM_LOCALES)[number][0];

function readLocaleCookie(): TelegramLocale {
  if (typeof document === "undefined") return "auto";
  const entry = document.cookie.split("; ").find((c) => c.startsWith("locale="));
  const value = entry?.slice("locale=".length);
  return value === "en" || value === "zh" || value === "hi" ? value : "auto";
}
  ...
  return (
    <main className="min-h-[100dvh] overflow-x-hidden bg-black text-white">
      <div className="telegram-app-shell mx-auto min-h-[100dvh] max-w-[560px] px-4 sm:px-6">
        ...
        {!selectedChatId ? <nav className="telegram-bottom-nav fixed left-1/2 z-40 grid w-[calc(100%-24px)] max-w-[536px] -translate-x-1/2 grid-cols-4 gap-1 rounded-full border border-white/[0.08] bg-black/80 p-1.5 shadow-[0_28px_80px_rgba(0,0,0,0.78)] backdrop-blur-2xl" aria-label="Main navigation">
        ...
        <div className="pb-8">
          ...
          {tab === "matches" ? selectedMatch ? <MatchDetail match={selectedMatch} pending={pending} onBack={closeDetail} onAction={matchAction} onChat={openChat} /> : <Matches matches={matches} openMatch={openMatch} /> : null}
...
function MatchDetail({ match, pending, onBack, onAction, onChat }: { match: MatchItem; pending: string | null; onBack: () => void; onAction: (id: string) => void; onChat: (id: string) => void }) {
  return <section>
...
function ChatView({ chat, ownerId, message, pending, onBack, onMessage, onSubmit, onAction, chatEndRef }: { chat: ChatDetail | null; ownerId: string; message: string; pending: string | null; onBack: () => void; onMessage: (value: string) => void; onSubmit: (event: FormEvent) => void; onAction: (action: "archive" | "block" | "report", reason?: string) => void; chatEndRef: React.RefObject<HTMLDivElement> }) {
  ...
  return <section className="flex flex-col telegram-chat-view">
...
function TelegramLanguage() {
  const [selected, setSelected] = useState<TelegramLocale>(() => readLocaleCookie());
  const [saving, setSaving] = useState(false);

  const choose = async (value: TelegramLocale) => {
    ...
    try {
      if (value === "auto") {
        ...
        await fetch("/api/locale", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ locale: value }) });
      }
      telegramHost()?.HapticFeedback?.notificationOccurred?.("success");
    } catch {
      setSelected(readLocaleCookie());
    } finally {
      setSaving(false);
    }
  };

  return <div className="space-y-2">{TELEGRAM_LOCALES.map(([value, label]) => <button key={value} type="button" disabled={saving} onClick={() => choose(value)} aria-pressed={selected === value} className={`flex h-14 w-full items-center justify-between rounded-full px-5 text-sm font-medium transition-colors disabled:opacity-60 ${selected === value ? "bg-white text-black" : "bg-black/[0.55] text-neutral-400"}`}><span>{label}</span>{selected === value ? <span aria-hidden="true">✓</span> : null}</button>)}</div>;
}
