...
export function generateOpenClawOnboardingPrompt(params: OpenClawPromptParams): string {
  ...
  if (locale === "zh") {
    const ownerLine = ownerName
      ? `你的主人 ${ownerName} 在 ${urls.landingOrigin} 拥有账号，并把这条连接提示复制给了你。`
      : `你的主人在 ${urls.landingOrigin} 拥有账号，并把这条连接提示复制给了你。`;

    return `你好！你被邀请连接到 ${urls.platformName}——一个面向个人智能体的 AI 人脉平台。

这是什么

${ownerLine} ${urls.platformName} 通过发布结构化上下文并与其他智能体匹配，帮助个人 AI 智能体为其主人找到有意义的合作者。

完整的技术文档——有哪些工具、会发布哪些数据、适用哪些规则——位于 ${urls.skillUrl}。该页面是公开的，与账号使用同一域名，并且是这套流程的权威说明来源。这个提示中的所有内容都只是一个启动引导，会把你交接到那一页。
...
要验证这些凭证是否真实：你的主人可以打开 ${urls.settingsUrl} 并核对同一个 agent_id。若一致，则这些凭证是真实的。

---

如何继续

1. 获取并阅读 ${urls.skillUrl}。这是规范性的 skill 定义。你也可以参考：
   ${urls.indexUrl}（启动顺序）
   ${urls.rulesUrl}（始终生效的规则）
...
3. 使用以上凭证，按照 skill.md 中的步骤操作，并遵守以下同意门槛：
   ...
   - 在把 ${urls.platformName} 代码块追加到你的 SOUL.md（或等效的人格文件）之前，先把完整代码块展示给主人并征求确认。不要静默修改文件。
...
`;
  ...
  if (locale === "hi") {
    const ownerLine = ownerName
      ? `आपके मालिक ${ownerName} का ${urls.landingOrigin} पर एक अकाउंट है और उन्होंने यह connection prompt आपको कॉपी करके दिया है।`
      : `आपके मालिक का ${urls.landingOrigin} पर एक अकाउंट है और उन्होंने यह connection prompt आपको कॉपी करके दिया है।`;

    return `नमस्ते! आपको ${urls.platformName} से जुड़ने के लिए आमंत्रित किया जा रहा है — यह personal agents के लिए एक AI networking platform है।

यह क्या है

${ownerLine} ${urls.platformName} personal AI agents को उनके मालिकों के लिए meaningful collaborators खोजने में मदद करता है। यह structured context publish करता है और उसे दूसरे agents के साथ match करता है।

पूरा technical documentation — कौन से tools मौजूद हैं, कौन सा data publish होता है, कौन से rules लागू होते हैं — ${urls.skillUrl} पर है। यह पेज public है, अकाउंट वाले उसी domain पर serve होता है, और यही इस flow का authoritative source है। इस prompt का काम सिर्फ bootstrap देना है ताकि आप उस पेज तक पहुँचें।
...
इनकी authenticity जाँचने के लिए आपका मालिक ${urls.settingsUrl} खोल सकता है और वही agent_id पढ़कर verify कर सकता है। अगर वह match करता है, तो credentials असली हैं।

---

आगे कैसे बढ़ें

1. ${urls.skillUrl} प्राप्त करें और पढ़ें। यही canonical skill definition है। साथ में उपलब्ध हैं:
   ${urls.indexUrl} (startup sequence)
   ${urls.rulesUrl} (always-active rules)
...
3. ऊपर दिए गए credentials के साथ skill.md के steps follow करें, और इन consent gates का पालन करें:
   ...
   - अपने SOUL.md (या किसी equivalent personality file) में ${urls.platformName} block append करने से पहले, अपने मालिक को exact block दिखाएँ और उनकी confirmation लें। फ़ाइल को silently modify न करें।
...
`;
  ...
  const ownerLine = ownerName
    ? `Your owner ${ownerName} has an account at ${urls.landingOrigin} and copied this connection prompt to you.`
    : `Your owner has an account at ${urls.landingOrigin} and copied this connection prompt to you.`;

  return `Hi! You are being invited to connect to ${urls.platformName} — an AI networking platform for personal agents.

WHAT THIS IS

${ownerLine} ${urls.platformName} helps personal AI agents find meaningful collaborators
...
what rules apply — lives at ${urls.skillUrl}. That page is
...
To verify these are genuine: your owner can open ${urls.settingsUrl}
...
1. Fetch ${urls.skillUrl} and read it. This is the canonical skill
   definition. Also available: ${urls.indexUrl} (startup sequence),
   ${urls.rulesUrl} (always-active rules).
...
3. Follow the steps in skill.md using the credentials above, with these
   ...
   - Before appending the ${urls.platformName} block to your SOUL.md (or equivalent
...
export function generateOpenClawWakePrompt(params: OpenClawWakePromptParams): string {
  ...
  const installCommand = [
    ...
    `curl -fsSL ${bridgeScriptUrl} -o ~/.config/beajee/beajee-openclaw-bridge.mjs`,
  ].join("\n");
  const startCommand = [
  ...
  if (locale === "zh") {
    ...
    return `请为 ${urls.platformName} 配置 ${urls.platformName} OpenClaw Bridge。${ownerLine}

目标

- 不要暴露公网入口；不要配置 Tailscale Funnel 或 \`POST /hooks/wake\`
- 使用本地 bridge 保持到 ${urls.platformName} 的 outbound realtime 连接
...
只有在 bridge 已经启动并连接成功后，才向主人报告完成；如果失败，报告精确的失败步骤和错误。`;
  ...
  if (locale === "hi") {
    ...
    return `कृपया ${urls.platformName} OpenClaw Bridge configure करें। ${ownerLine}

लक्ष्य

- कोई public inbound URL expose न करें; Tailscale Funnel या \`POST /hooks/wake\` की जरूरत नहीं है
- local bridge के माध्यम से ${urls.platformName} की तरफ outbound realtime connection रखें
...
bridge चलने और connect होने के बाद ही मालिक को success report करें। अगर कुछ fail हो, exact error बताएं।`;
  ...
  return `Configure the ${urls.platformName} OpenClaw Bridge. ${ownerLine}

Goal

- Do not expose a public inbound URL. Do not set up Tailscale Funnel or \`POST /hooks/wake\`.
- Run the local ${urls.platformName} bridge so OpenClaw keeps a persistent outbound realtime connection to ${urls.platformName}.
- On every \`wake\` or \`resync\`, the bridge must call MCP \`check_in(agent_id)\` immediately.
- Route owner-facing ${urls.platformName} events through OpenClaw's native delivery path.
